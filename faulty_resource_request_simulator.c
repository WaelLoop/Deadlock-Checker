...
void* fault_simulator(void* pr_id){
    while(1){
        printf("Running fault simulator\n");
        //uniformally selecting a random resource
        int randomResource = rand() % (numResources);
        int chance = rand() % 2;
        //50% chance of decreasing the randomly select resource
        if(chance == 1){
            pthread_mutex_lock(&mutex);
            printf("Resource %d has been decreased by 1\n", randomResource);
    ...
    while(1){
        ...
        for(int i=0;i<numResources;i++){
            ...
            for(int j=0;j<numProcesses;j++){
                if(need[j][i] <= avail[i] && need[j][i] != -1) {
                    flag = 0;//it not
                }
            }
            if(flag != 0){
                printf("Deadlcok will occur as processes request more resources, exiting...\n");
                exit(1);
            }
        }
        pthread_mutex_unlock(&mutex);
...
int main(){
    ...
    pthread_create(&fault,NULL,fault_simulator,NULL);
